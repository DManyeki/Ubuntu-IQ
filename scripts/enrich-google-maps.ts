...
async function main() {
    console.log(`🗺️ Google Maps Enrichment (Full Production Run: Pending Only)`);
    console.log(`   Batch size: All Pending`);
    ...
    if (fs.existsSync(URLS_FILE)) {
        ...
        console.log(`Loaded ${priorityIds.size} priority institutions from URL finder.\n`);
    } else {
        console.log("Warning: tvet_urls_found.json not found. Proceeding with all institutions.\n");
    }
    ...
    // Filter to only those not yet done AND in priority list (if valid)
    const pending = institutions.filter(inst => {
        const isNotProcessed = !processedIds.has(inst.id);
        const isPriority = priorityIds.size > 0 ? priorityIds.has(inst.id) : true;
        return isNotProcessed && isPriority;
    });

    console.log(`Found ${institutions.length} total in DB.`);
    if (priorityIds.size > 0) console.log(`Filtered to ${priorityIds.size} priority items.`);
    console.log(`Processed ${processedIds.size} already.`);
    console.log(`Queued ${pending.length} for processing.\n`);

    if (pending.length === 0) {
        console.log("All priority institutions processed! 🎉");
        return;
    }
